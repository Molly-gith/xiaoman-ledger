...
export default function Home() {
  ...
  const budgetCard = metrics && plan && <section className="section-block budget-garden"><div className="section-title"><h2><StoryIcon name="jar"/>这个周期的钱</h2><button onClick={() => setModal("cycle")}>调整预算</button></div><div className="budget-row"><span>日常已花 / 可花总额</span><b>{money(metrics.variableSpend)} / {money(metrics.variableBudget)}</b></div><span className="progress" role="progressbar" aria-label="日常花销预算使用进度" aria-valuenow={Math.min(100, Math.max(0, metrics.budgetPercent ?? 0))}><i style={{ width: `${Math.min(100, Math.max(0, metrics.budgetPercent ?? 0))}%` }} /></span><div className="budget-foot"><span>{metrics.budgetPercent === null ? "还没有安排日常预算" : `已用 ${metrics.budgetPercent.toFixed(1)}%`}</span><span>账单预留还剩 {money(metrics.reserveRemaining)}</span></div><div className="savings-line"><StoryIcon name="leaf"/><span>计划先存下 <b>{money(plan.plannedSavings)}</b><small>这是计划，还不代表实际已存</small></span></div></section>;
  return <main className="app-shell"><fieldset className="app-content" disabled={busy} inert={showDialog}>
    {tab === "home" && <><header className="topbar"><div className="storybook-brand"><StoryIcon name="leaf"/><div><h1>小满</h1><p>把日子，过成喜欢的样子</p></div></div><button className="avatar" onClick={() => setModal("data")} aria-label="本地数据与备份"><StoryIcon name="user"/></button></header>
      {cycle && metrics && <><section className="balance-card"><div className="balance-top"><span>这个周期，还能安心花</span><span className="card-mark"><StoryIcon name="lock"/>仅存此设备</span></div><strong data-testid="safe-to-spend">{metrics.unresolvedCount ? "待核对旧账" : expired ? "请开启新周期" : money(metrics.safeToSpend)}</strong><p className="balance-explanation">已经为储蓄和账单留好了钱。</p><div className="balance-stats"><div><span>{expired ? "上个周期已结束" : "距下次发薪"}</span><b>{remainingDays(cycle, today)} 天</b></div><div><span>财务周期</span><b>{cycle.startDate.slice(5)} — {cycle.endDate.slice(5)}</b></div></div></section>
      {expired && <button className="primary" onClick={() => setModal("cycle")}>确认收入，开启新周期</button>}
      {!!metrics.unresolvedCount && <div className="gentle-tip"><p>{metrics.unresolvedCount} 笔旧支出尚需核对日期、周期或支出来源。请在账单中编辑确认，完成后再显示安心可花。</p><button onClick={() => { setTab("bills"); setPeriod("year"); setFilter("expense"); }}>核对账单</button></div>}
